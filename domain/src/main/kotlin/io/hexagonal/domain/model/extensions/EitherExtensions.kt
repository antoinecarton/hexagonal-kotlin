...
fun <T> Either.Companion.trying(block: () -> T): DResult<T> =
    try {
        ...
        DomainError.Unknown(e.message ?: "", e).left()
    }

fun <T> Either.Companion.trying(block: () -> T, ifFail: (Exception) -> DomainError): DResult<T> =
    try {
        block().right()
    } catch (e: Exception) {
...
suspend fun <T> Either.Companion.sTrying(block: suspend () -> T, ifFail: (Exception) -> DomainError): DResult<T> =
    try {
        block().right()
    } catch (e: Exception) {
        ifFail.invoke(e).left()
